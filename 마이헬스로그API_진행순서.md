...
- [ ] 제출자 정보 작성 (이름, 소속, GitHub URL)
- [ ] Python 가상환경 생성 & FastAPI, uvicorn 설치
- [ ] GitHub 저장소 생성 (Public)

## 1단계. 데이터 모델 & 기본 CRUD (user 필드 포함)

- [ ] 기록 데이터 모델(Pydantic) 정의 — date, weight, height, systolic, diastolic, blood_sugar, steps, sleep_hours, memo, **user**
- [ ] `POST /records` 구현 — 기록 추가
- [ ] `GET /records`, `GET /records/{id}` 구현 — 목록/단건 조회 (없으면 404)
  - `GET /records?user=...` 로 사용자별 필터링 지원
- [ ] `DELETE /records/{id}` 구현
- [ ] 첫 커밋 & push
...
- [ ] BMI 계산 함수 작성 — 몸무게(kg) ÷ 키(m)²
- [ ] BMI 분류 (저체중/정상/과체중/비만)
- [ ] 혈압 분류 (정상/주의/고혈압)
- [ ] 혈당 분류 (정상/공복혈당장애/당뇨 의심)
- [ ] warnings 생성 로직 (비만·고혈압·당뇨 의심 시 메시지 추가, 해당 없으면 빈 배열)
- [ ] 걸음 수 등급 (부족/적정/우수)
- [ ] 수면 분석 (평균 vs 권장 수면 비교)
- [ ] 조회 응답에 bmi·bmi_category·bp_category·sugar_category·warnings·걸음수 등급·수면 분석 포함
- [ ] `PUT /records/{id}` 구현
- [ ] 커밋 & push
...
- [ ] `GET /search` 구현 — 날짜 범위(start, end) 검색, user별 필터링
- [ ] `GET /stats` 구현 — 평균 체중 등 통계, user별 필터링
- [ ] 주간 리포트 — 최근 7일 평균, 지난주 대비 변화 (user별)
- [ ] 기록을 JSON 파일로 저장 (user 정보 포함) → 서버 재시작해도 유지되도록 처리
